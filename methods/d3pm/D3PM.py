...
import methods.d3pm.utils as utils
import torch
import numpy as np
...
class CategoricalDiffusion(nn.Module):
    ...
    def _at(self, a, t, x):
        ...
        Returns:
          a[t, x]: jnp.ndarray: Jax _at_onehotarray.
        """
    ...
    def _at_onehot(self, a, t, x):
        ...
        a_t = torch.index_select(a, dim=0, index=t)
        # print('--------- in _at_onehot -------------- ')
        # print(a.shape, a_t.shape, x.shape, x.sum())
        # print('x:', x)
        assert a_t.shape == (x.shape[0], self.num_pixel_vals, self.num_pixel_vals)
    ...
    def p_logits(self, model_fn, state, *, x, t):
        """Compute logits of p(x_{t-1} | x_t)."""
        assert t.shape == (x.shape[0],)
        model_output = model_fn(x, t, state)
    ...
    # @torch.no_grad()
    def p_sample_loop(self, model_fn, state, shape,
        ...
        if return_x_init:
            return x_init, x.detach(), probs.squeeze(1)
        else:
            return x.detach(), probs.squeeze(1)
