...
def setup_my_logger(
    ...
    """
    Set up my_logger to have some reasonable default settings.
    ...
    if variant is not None:
        my_logger.log("Variant:")
        my_logger.log(json.dumps(dict_to_safe_json(variant), indent=2))
        variant_log_path = osp.join(log_dir, variant_log_file)
        my_logger.log_variant(variant_log_path, variant)

    tabular_log_path = osp.join(log_dir, tabular_log_file)
    text_log_path = osp.join(log_dir, text_log_file)

    my_logger.add_text_output(text_log_path)
    if first_time:
        my_logger.add_tabular_output(tabular_log_path)
    else:
        my_logger._add_output(tabular_log_path, my_logger._tabular_outputs,
                           my_logger._tabular_fds, mode='a')
        for tabular_fd in my_logger._tabular_fds:
            my_logger._tabular_header_written.add(tabular_fd)
    my_logger.set_snapshot_dir(log_dir)
    my_logger.set_snapshot_mode(snapshot_mode)
    my_logger.set_snapshot_gap(snapshot_gap)
    my_logger.set_log_tabular_only(log_tabular_only)
    exp_name = str(log_dir.split("/")[-2]) + '/' + str(log_dir.split("/")[-1])
    my_logger.push_prefix("[%s] " % exp_name)
